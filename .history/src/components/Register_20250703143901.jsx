...
const Register = ({ children }) => {
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    role: 'operator' // default role changed from 'user' to 'operator'
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { user, error } = await signUp(formData.email, formData.password, formData.name, formData.role);
      if (error) {
        toast({
          ...
          <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                value={formData.role}
                onChange={handleInputChange}
                className={`w-full pl-4 pr-4 py-4 border-2 rounded-2xl bg-[hsl(var(--background))/60] focus:bg-[hsl(var(--background))] focus:ring-4 focus:ring-[hsl(var(--caterpillar-yellow))/30] focus:border-[hsl(var(--caterpillar-yellow))] transition-all duration-300 ${{
                  'border-rose-300 bg-rose-50/50': errors.role,
                  'border-[hsl(var(--caterpillar-yellow))]': !errors.role
                }}`}
              >
                <option value="operator">Operator</option>
                <option value="admin">Admin</option>
              </select>
              {errors.role && <p className="text-rose-500 text-sm flex items-center gap-1">
                <span className="w-1 h-1 bg-rose-500 rounded-full"></span>
                {errors.role}
              </p>}
            </div>
